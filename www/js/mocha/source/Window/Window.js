...
MochaUI.Windows.windowOptions = {
	...
	onResize:          function(item){
        // Save window width and height in cookie
        imt_w = $(item).getStyle('width');
        imt_h = $(item).getStyle('height');
        var myCookie   = Cookie.write( $(item).id + '_w' , imt_w , {duration: 60});
        var myCookie2  = Cookie.write( $(item).id + '_h' , imt_h , {duration: 60});
    },
	onMinimize:        $empty,
	onMaximize:        $empty,
	onRestore:         $empty,
	...
	*/
	newWindow: function(properties){ // options is not doing anything
		...
		this.fireEvent('onBeforeBuild');
            ...
            //
            // Begin load window params from cookie
            //
            var cw_width = this.options.id + '_w';
            var cw_height = this.options.id + '_h';

            var c_win_width = Cookie.read(cw_width);
            var c_win_height = Cookie.read(cw_height);

            if (c_win_width != null && c_win_height != null)
            {
               this.options.width = c_win_width.toInt() - 10;
               this.options.height = c_win_height.toInt() - 60;
            }
            //

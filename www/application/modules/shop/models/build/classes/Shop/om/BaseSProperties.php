...
abstract class BaseSProperties extends ShopBaseObject  implements Persistent
{
	...
	/**
	 * Set the value of [show_on_site] column.
	 * 
	 * @param      boolean $v new value
	 * @return     SProperties The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->show_on_site = ($row[$startcol + 4] !== null) ? (boolean) $row[$startcol + 4] : null;
			$this->show_in_compare = ($row[$startcol + 5] !== null) ? (boolean) $row[$startcol + 5] : null;
			...
			return $startcol + 8; // 8 = SPropertiesPeer::NUM_COLUMNS - SPropertiesPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[4] => $this->getShowOnSite(),
			$keys[5] => $this->getShowInCompare(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[4], $arr)) $this->setShowOnSite($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setShowInCompare($arr[$keys[5]]);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setCsvName($this->csv_name);
		$copyObj->setActive($this->active);
		$copyObj->setShowOnSite($this->show_on_site);

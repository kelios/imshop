...
abstract class BaseSProductVariants extends ShopBaseObject  implements Persistent
{
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = SProductVariantsPeer::NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 7; // 7 = SProductVariantsPeer::NUM_COLUMNS - SProductVariantsPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(SProductVariantsPeer::ID) ) {
					...
					$affectedRows += SProductVariantsPeer::doUpdate($this, $con);
				}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SProductVariantsPeer::NAME)) $criteria->add(SProductVariantsPeer::NAME, $this->name);

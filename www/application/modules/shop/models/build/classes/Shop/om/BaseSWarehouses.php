 ...
 */
abstract class BaseSWarehouses extends ShopBaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'SWarehousesPeer';
	 ...
	 * @return     SWarehouses The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = SWarehousesPeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = SWarehousesPeer::NAME;
		}
	...
	public function setAddress($v)
	{
		...
		if ($this->address !== $v) {
			$this->address = $v;
			$this->modifiedColumns[] = SWarehousesPeer::ADDRESS;
		}
	...
	public function setPhone($v)
	{
		...
		if ($this->phone !== $v) {
			$this->phone = $v;
			$this->modifiedColumns[] = SWarehousesPeer::PHONE;
		}
	...
	public function setDescription($v)
	{
		...
		if ($this->description !== $v) {
			$this->description = $v;
			$this->modifiedColumns[] = SWarehousesPeer::DESCRIPTION;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 5; // 5 = SWarehousesPeer::NUM_COLUMNS - SWarehousesPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating SWarehouses object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = SWarehousesPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				SWarehousesPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(SWarehousesPeer::ID) ) {
					...
					$affectedRows = SWarehousesPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = SWarehousesPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = SWarehousesPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = SWarehousesPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SWarehousesPeer::ID)) $criteria->add(SWarehousesPeer::ID, $this->id);
		if ($this->isColumnModified(SWarehousesPeer::NAME)) $criteria->add(SWarehousesPeer::NAME, $this->name);
		if ($this->isColumnModified(SWarehousesPeer::ADDRESS)) $criteria->add(SWarehousesPeer::ADDRESS, $this->address);
		if ($this->isColumnModified(SWarehousesPeer::PHONE)) $criteria->add(SWarehousesPeer::PHONE, $this->phone);
		if ($this->isColumnModified(SWarehousesPeer::DESCRIPTION)) $criteria->add(SWarehousesPeer::DESCRIPTION, $this->description);
	...
	{
		$criteria = new Criteria(SWarehousesPeer::DATABASE_NAME);
		$criteria->add(SWarehousesPeer::ID, $this->id);
	 ...
	 *
	 * @return     SWarehousesPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new SWarehousesPeer();
		}
		return self::$peer;
	}
	 ...
	 * If this SWarehouses is new, it will return
	 ...
	 * @return     PropelCollection|array SWarehouseData[] List of SWarehouseData objects
	 */
	public function getSWarehouseDatasJoinSProducts($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = SWarehouseDataQuery::create(null, $criteria);
	...
	{
		return (string) $this->exportTo(SWarehousesPeer::DEFAULT_STRING_FORMAT);
	}

...
abstract class BaseShopDiscounts extends ShopBaseObject  implements Persistent
{
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 12; // 12 = ShopDiscountsPeer::NUM_COLUMNS - ShopDiscountsPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(ShopDiscountsPeer::ID) ) {
					...
					$affectedRows = ShopDiscountsPeer::doUpdate($this, $con);
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setDateStop($this->date_stop);
		$copyObj->setDiscount($this->discount);
		$copyObj->setUserGroup($this->user_group);
		$copyObj->setMinPrice($this->min_price);
		$copyObj->setMaxPrice($this->max_price);

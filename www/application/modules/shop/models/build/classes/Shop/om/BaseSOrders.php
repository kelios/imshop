...
abstract class BaseSOrders extends ShopBaseObject  implements Persistent
{
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->paid = ($row[$startcol + 6] !== null) ? (boolean) $row[$startcol + 6] : null;
			$this->user_full_name = ($row[$startcol + 7] !== null) ? (string) $row[$startcol + 7] : null;
			$this->user_email = ($row[$startcol + 8] !== null) ? (string) $row[$startcol + 8] : null;
			$this->user_phone = ($row[$startcol + 9] !== null) ? (string) $row[$startcol + 9] : null;
			$this->user_deliver_to = ($row[$startcol + 10] !== null) ? (string) $row[$startcol + 10] : null;
			$this->user_comment = ($row[$startcol + 11] !== null) ? (string) $row[$startcol + 11] : null;
			$this->date_created = ($row[$startcol + 12] !== null) ? (int) $row[$startcol + 12] : null;
			$this->date_updated = ($row[$startcol + 13] !== null) ? (int) $row[$startcol + 13] : null;
			$this->user_ip = ($row[$startcol + 14] !== null) ? (string) $row[$startcol + 14] : null;
			$this->resetModified();
			...
			return $startcol + 15; // 15 = SOrdersPeer::NUM_COLUMNS - SOrdersPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $includeForeignObjects = false)
	{
		...
		$result = array(
			...
			$keys[6] => $this->getPaid(),
			$keys[7] => $this->getUserFullName(),
			$keys[8] => $this->getUserEmail(),
			$keys[9] => $this->getUserPhone(),
			$keys[10] => $this->getUserDeliverTo(),
			$keys[11] => $this->getUserComment(),
			$keys[12] => $this->getDateCreated(),
			$keys[13] => $this->getDateUpdated(),
			$keys[14] => $this->getUserIp(),
		);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[7], $arr)) $this->setUserFullName($arr[$keys[7]]);
		if (array_key_exists($keys[8], $arr)) $this->setUserEmail($arr[$keys[8]]);
		if (array_key_exists($keys[9], $arr)) $this->setUserPhone($arr[$keys[9]]);
		if (array_key_exists($keys[10], $arr)) $this->setUserDeliverTo($arr[$keys[10]]);
		if (array_key_exists($keys[11], $arr)) $this->setUserComment($arr[$keys[11]]);
		if (array_key_exists($keys[12], $arr)) $this->setDateCreated($arr[$keys[12]]);
		if (array_key_exists($keys[13], $arr)) $this->setDateUpdated($arr[$keys[13]]);
		if (array_key_exists($keys[14], $arr)) $this->setUserIp($arr[$keys[14]]);
	}

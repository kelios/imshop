...
abstract class BaseSPaymentMethods extends ShopBaseObject  implements Persistent
{
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = SPaymentMethodsPeer::NAME;
		}
	...
	public function setDescription($v)
	{
		...
		if ($this->description !== $v) {
			$this->description = $v;
			$this->modifiedColumns[] = SPaymentMethodsPeer::DESCRIPTION;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->payment_system_name = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			...
			return $startcol + 7; // 7 = SPaymentMethodsPeer::NUM_COLUMNS - SPaymentMethodsPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(SPaymentMethodsPeer::ID) ) {
					...
					$affectedRows += SPaymentMethodsPeer::doUpdate($this, $con);
				}
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $includeForeignObjects = false)
	{
		...
		$result = array(
			...
			$keys[5] => $this->getPaymentSystemName(),
			$keys[6] => $this->getPosition(),
		);
		if ($includeForeignObjects) {
			if (null !== $this->aCurrency) {
				$result['Currency'] = $this->aCurrency->toArray($keyType, $includeLazyLoadColumns, true);
			}
		}
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[5], $arr)) $this->setPaymentSystemName($arr[$keys[5]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SPaymentMethodsPeer::ID)) $criteria->add(SPaymentMethodsPeer::ID, $this->id);
		if ($this->isColumnModified(SPaymentMethodsPeer::NAME)) $criteria->add(SPaymentMethodsPeer::NAME, $this->name);
		if ($this->isColumnModified(SPaymentMethodsPeer::DESCRIPTION)) $criteria->add(SPaymentMethodsPeer::DESCRIPTION, $this->description);
		if ($this->isColumnModified(SPaymentMethodsPeer::ACTIVE)) $criteria->add(SPaymentMethodsPeer::ACTIVE, $this->active);
		if ($this->isColumnModified(SPaymentMethodsPeer::CURRENCY_ID)) $criteria->add(SPaymentMethodsPeer::CURRENCY_ID, $this->currency_id);
		if ($this->isColumnModified(SPaymentMethodsPeer::PAYMENT_SYSTEM_NAME)) $criteria->add(SPaymentMethodsPeer::PAYMENT_SYSTEM_NAME, $this->payment_system_name);
		if ($this->isColumnModified(SPaymentMethodsPeer::POSITION)) $criteria->add(SPaymentMethodsPeer::POSITION, $this->position);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setPaymentSystemName($this->payment_system_name);
	...
	public function getSDeliveryMethodss($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collSDeliveryMethodss || null !== $criteria) {
			if ($this->isNew() && null === $this->collSDeliveryMethodss) {
				...
				$collSDeliveryMethodss = SDeliveryMethodsQuery::create(null, $criteria)
					->filterByPaymentMethods($this)
					->find($con);
				if (null !== $criteria) {
	 ...
	 *
	 * @param      SDeliveryMethods $sDeliveryMethods The ShopDeliveryMethodsSystems object to relate
	 * @return     void
	 */
	public function addSDeliveryMethods($sDeliveryMethods)
	{

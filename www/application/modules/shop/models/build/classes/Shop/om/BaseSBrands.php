 ...
 */
abstract class BaseSBrands extends ShopBaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'SBrandsPeer';
	 ...
	 * @return     SBrands The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = SBrandsPeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = SBrandsPeer::NAME;
		}
	...
	public function setUrl($v)
	{
		...
		if ($this->url !== $v) {
			$this->url = $v;
			$this->modifiedColumns[] = SBrandsPeer::URL;
		}
	...
	public function setDescription($v)
	{
		...
		if ($this->description !== $v) {
			$this->description = $v;
			$this->modifiedColumns[] = SBrandsPeer::DESCRIPTION;
		}
	...
	public function setMetaTitle($v)
	{
		...
		if ($this->meta_title !== $v) {
			$this->meta_title = $v;
			$this->modifiedColumns[] = SBrandsPeer::META_TITLE;
		}
	...
	public function setMetaDescription($v)
	{
		...
		if ($this->meta_description !== $v) {
			...
			$this->modifiedColumns[] = SBrandsPeer::META_DESCRIPTION;
		}
	...
	public function setMetaKeywords($v)
	{
		...
		if ($this->meta_keywords !== $v) {
			...
			$this->modifiedColumns[] = SBrandsPeer::META_KEYWORDS;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->meta_title = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			...
			return $startcol + 7; // 7 = SBrandsPeer::NUM_COLUMNS - SBrandsPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating SBrands object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = SBrandsPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				SBrandsPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(SBrandsPeer::ID) ) {
					...
					$affectedRows = SBrandsPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = SBrandsPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = SBrandsPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = SBrandsPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
			$keys[2] => $this->getUrl(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SBrandsPeer::ID)) $criteria->add(SBrandsPeer::ID, $this->id);
		if ($this->isColumnModified(SBrandsPeer::NAME)) $criteria->add(SBrandsPeer::NAME, $this->name);
		if ($this->isColumnModified(SBrandsPeer::URL)) $criteria->add(SBrandsPeer::URL, $this->url);
		if ($this->isColumnModified(SBrandsPeer::DESCRIPTION)) $criteria->add(SBrandsPeer::DESCRIPTION, $this->description);
		if ($this->isColumnModified(SBrandsPeer::META_TITLE)) $criteria->add(SBrandsPeer::META_TITLE, $this->meta_title);
		if ($this->isColumnModified(SBrandsPeer::META_DESCRIPTION)) $criteria->add(SBrandsPeer::META_DESCRIPTION, $this->meta_description);
		if ($this->isColumnModified(SBrandsPeer::META_KEYWORDS)) $criteria->add(SBrandsPeer::META_KEYWORDS, $this->meta_keywords);
	...
	{
		$criteria = new Criteria(SBrandsPeer::DATABASE_NAME);
		$criteria->add(SBrandsPeer::ID, $this->id);
	 ...
	 * If this SBrands is new, it will return
	 ...
	 * @return     PropelCollection|array SProducts[] List of SProducts objects
	 */
	public function getSProductssJoinMainCategory($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = SProductsQuery::create(null, $criteria);
		$query->joinWith('MainCategory', $join_behavior);
	...
	{
		return (string) $this->exportTo(SBrandsPeer::DEFAULT_STRING_FORMAT);
	}

...
/**
 * Base class that represents a row from the 'shop_user_profile' table.
 *
 * 
 *
...
abstract class BaseSUserProfile extends ShopBaseObject  implements Persistent
{
	...
	public function setUserId($v)
	{
		...
		if ($this->user_id !== $v) {
			$this->user_id = $v;
			$this->modifiedColumns[] = SUserProfilePeer::USER_ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = SUserProfilePeer::NAME;
		}
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(SUserProfilePeer::ID) ) {
					...
					$affectedRows = SUserProfilePeer::doUpdate($this, $con);
				}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SUserProfilePeer::ID)) $criteria->add(SUserProfilePeer::ID, $this->id);
		if ($this->isColumnModified(SUserProfilePeer::USER_ID)) $criteria->add(SUserProfilePeer::USER_ID, $this->user_id);
		if ($this->isColumnModified(SUserProfilePeer::NAME)) $criteria->add(SUserProfilePeer::NAME, $this->name);

...
<form method="post" action="{$ADMIN_URL}properties/edit/{echo $model->getId()}"  style="width:100%">
    ...
    <div class="form_text">{echo $model->getLabel('CsvName')}:</div>
    <div class="form_input">
        <input type="text" name="CsvName" value="{echo ShopCore::encode($model->getCsvName())}" class="textbox_long" /> <span class="required">*</span>
    ...
    <div class="form_input">
        ...
        <br/>
        <label><input type="checkbox" {if $model->getShowOnSite() == true}checked="checked"{/if} value="1" name="ShowOnSite" />{echo $model->getLabel('ShowOnSite')}</label>
        <br/>

...
<div id="sortable" style="clear:both;">
		...
		<tr id="productRow{echo $p->getId()}" class="row">
            ...
            <td>
                ...
                onClick="ajaxShop('products/edit/{echo $p->getId()}?redirect={$cur_uri_str}'); return false;">{truncate(ShopCore::encode($p->getName()),100)}</a>
            </td>
            <td>
...
<div class="footer_panel" align="right" id="productsListFooter" style="display:none;">
    <input id="footerImageButton" class="Tick" value="&nbsp;" type="button" onclick="shopProductsList_changeActive('{echo $CI->uri->uri_string()}?{http_build_query(ShopCore::$_GET)}');" title="Изменить 'Активен'">
    <input id="footerImageButton" class="Star" value="&nbsp;" type="button" onclick="shopProductsList_changeHit('{echo $CI->uri->uri_string()}?{http_build_query(ShopCore::$_GET)}');" title="Изменить 'Хит'">

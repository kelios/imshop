...
<form method="post" action="{$ADMIN_URL}/deliverymethods/edit/{echo $model->getId()}"  style="width:100%">
    ...
    <div class="form_input">
        <input type="text" name="Price" value="{echo ShopCore::encode($model->getPrice())}" class="textbox_long" /> {$CS}
    </div>
    <div class="form_overflow"></div>

    <div class="form_text">{echo $model->getLabel('FreeFrom')}:</div>
    <div class="form_input">
        <input type="text" name="FreeFrom" value="{echo ShopCore::encode($model->getFreeFrom())}" class="textbox_long" /> {$CS}
    </div>
    <div class="form_overflow"></div>
